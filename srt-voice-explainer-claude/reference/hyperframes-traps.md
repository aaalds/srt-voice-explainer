...
## 子 composition 的样式和字体要各自带一份

`<template>` 里的 `<style>` 是这个子 composition 的全部样式来源——
它**不继承** `index.html` 里的 `@font-face`。少了就报 `font_family_without_font_face`，
一个子 comp 一条（七章七条）。每个 `chN.html` 的 `<style>` 里都要重复写一遍
`@font-face`（三个字重都要），`<script>` 同理必须在 `<template>` 内。

根元素用 **ID 选择器**写样式（`#root { … }`），不要用类选择器——
类名在拼装后的作用域里不一定命中。
...
## transform 的对象必须是块级且有尺寸

`scaleX` / `scaleY` 缩放的是**既有几何**，不凭空造几何：

- `position:absolute` 但没写 `width` 的元素，宽度是 0，`scaleX(0→1)` 渲染结果还是 0；
- 行内元素（默认的 `<span>`）上的 transform 静默不生效，要 `display: block`。

两种情况都**不报错、不 warning**，`check` 全绿，只有抽帧目检能发现（见 pitfalls R10）。
写进度条、下划线、色条这类"长出来"的元素时，先确认它有确定的宽高。

## 实拍素材：`<video>` 的挂法

素材接入的完整流程见 `user-footage.md`，这里只记契约：

```html
<div class="layer" id="ch5-l-video">        <!-- 未计时的父层 -->
  <video id="ch5-shot" src="assets/shot-full.mp4"
         data-start="0" data-duration="12" data-track-index="3"
         muted playsinline
         style="position:absolute; left:0; top:0; width:880px; height:662px"></video>
</div>
```

- `<video>` 必须是**未计时父层的直接子元素**，自己带 `data-start` / `data-duration`。
  从素材中段开始播就加 `data-media-start="8"`。
- **不要给它 `class="clip"`，也不要预设 `opacity: 0`。** 可见性由框架接管；
  自己压透明度会让它永远不出画（踩过一次）。
  由此产生的 `timed_element_missing_visibility_hidden` 是可以接受的 info。
- 必须 `muted playsinline`，并显式写 `width` / `height`。
- 同一个 `src` 用在两个不同 `data-start` 上会报 `duplicate_media_discovery_risk`，
  官方标注为 benign——但要抽帧确认两段都真的出画了才能接受。

## `check` 的快照不解码视频

快照里媒体位置是一整块黑，这**不是缺陷**（pitfalls R15）。
要确认实拍出画，用一次草稿渲染在对应时间点抽帧。
不要为了让快照"好看"去改一个只存在于检查工具里的问题。
...
不是所有 warning 都要清零，但**每一条都要逐项审查并写进 QA 报告**。本管线接受过这几类：

- `composition_file_too_large`——信息可视化本身复杂，继续拆分会引入跨文件挂载风险
  （见 R1），而不会改善成片。
- `container_overflow`（背景径向辉光）——`design.md` 明确允许装饰性元素进入出血区。
- `timed_element_missing_visibility_hidden`（`<video>`）——可见性由框架接管，
  加 `opacity:0` 反而让它永不出画。
- `duplicate_media_discovery_risk`（同一素材两个 `data-start`）——官方标注 benign，
  且已抽帧确认两段都出画。
